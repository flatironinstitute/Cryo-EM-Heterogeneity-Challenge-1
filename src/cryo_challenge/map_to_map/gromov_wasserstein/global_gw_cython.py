...
from cryo_challenge.map_to_map.gromov_wasserstein.gw_weighted_voxels import (
    setup_volume_and_distance,
    precision,
)
from update_D import update_D_c
...
def init_extreme_points(lower_bounds, upper_bounds):
    ...
    D_row_index, D_length = np.unique(D_row, return_counts=True)

    D_dict = {"row": D_row.astype(np.int64),
            "col": D_col.astype(np.int64),
                "length": D_length.astype(np.int64),
            "row_index": D_row_index.astype(np.int64)}
...
def extreme_points_update(E, B, D, A, b, do_optimize_with_sparse):
    ...
    D_col = D["col"]
    D_length = D["length"]
    D_row_index = D["row_index"]
    
    if isinstance(D_length, list):
        D_col_index = np.cumsum([0] + D_length)
    else:
        D_col_index = np.cumsum(np.r_[np.array([0], dtype=np.int64), D_length])

    A = A.ravel()
    b = b.ravel()[0]

    M = E @ A
    arange = np.arange(E.shape[0])

    new_constraint_values = M - b
    infeasible_indices_mask = (new_constraint_values > 0)
    infeasible_indices = arange[infeasible_indices_mask]

    new_extreme_points = []
    C = []
    O = []

    corrected_indices = np.cumsum(~infeasible_indices_mask)-1

    for i in infeasible_indices:
        index = bisect.bisect_left(D_row_index, i)
        feasible_adjacent_indices = D_col[D_col_index[index] : D_col_index[index+1]]
        for j in feasible_adjacent_indices:
            if not infeasible_indices_mask[j]:
                lamb = new_constraint_values[j] / (M[j] - M[i])
                new_point = (1-lamb) *  E[j] + lamb * E[i]
                new_extreme_points.append(new_point)
                C.append(B[:, i] & B[:, j])
                O.append(corrected_indices[j])
    ...
    if new_extreme_points:
        ...
        E = E[~infeasible_indices_mask]
        B = B[:, ~infeasible_indices_mask]
        
        E = np.r_[E, P]
        b_row = np.zeros((1, B.shape[1]), dtype=bool)
        b_row = np.column_stack((b_row, np.ones((1, len(new_extreme_points)), dtype=bool)))
        ...
        if do_optimize_with_sparse:
            ...
            N = np.ascontiguousarray(N_sparse.toarray())
        else:
            N_dense = C_int.T @ C_int == r - 2
            np.fill_diagonal(N_dense, True)
            N = N_dense
        
        infeasible_indices_mask = infeasible_indices_mask.astype(np.int64)
        corrected_indices = corrected_indices.astype(np.int64)
        O = O.astype(np.int64)
        N = N.astype(np.int64)

        D = update_D_c(D_row, D_col, D_length,
                       infeasible_indices_mask,
                       corrected_indices,
                       O, N)
