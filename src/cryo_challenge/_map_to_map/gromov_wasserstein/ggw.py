...
def optimal_extreme_point(E):
    ...
    Fi: Minimum value
    """
    F = -E[:, 0] - np.sum(E[:, 1:] ** 2, axis=1)  # explicitly evaluates the objective
    index = np.argmin(F)  # min by explicit enumeration of all extreme points
...
def gw_global(
    ...
    for niter in range(IterMax):
        ...
        wW_cache, l_bound = optimal_extreme_point(E)  # TODO: changes? # print(E.shape)
