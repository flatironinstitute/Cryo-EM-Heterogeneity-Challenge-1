...
def _compute_bfactor_filter(b_factor, box_size, voxel_size):
    """
...
def apply_bfactor_filter(volumes, bfactor, voxel_size, in_place=False):
    """
    Apply a B-factor correction to the volumes. Positive B-factors correspond to B-factor sharpening, while negative B-factors correspond to B-factor blurring.
    ...
    b_factor_scaling = _compute_bfactor_filter(bfactor, volumes.shape[-1], voxel_size)
